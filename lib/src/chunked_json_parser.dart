...
abstract class _ChunkedJsonParser<T> {
  ...
  List<JsonEvent> get result => listener.result;
  ...
  int parsePartialString(int position, int partialState) {
    ...
    for (int i = count; i < 4; i++, position++) {
      if (position == chunkEnd) {
        return chunkStringEscapeU(i, value);
      }
      int char = getChar(position);
      int digit = parseHexDigit(char);
      if (digit < 0) {
        fail(position, "Invalid hex digit");
      }
  ...
  int parseKeywordPrefix(int position, String chars, int type) {
    ...
    while (++position < length) {
      int char = getChar(position);
      if (char != chars.codeUnitAt(count)) {
        fail(start);
      }
      count++;
    }
  ...
  int parseString(int position) {
    ...
    while (position < end) {
      ...
      if (char == BACKSLASH) {
        ...
        if (start < sliceEnd) {
          addSliceToString(start, sliceEnd);
        }
  ...
  int parseNumber(int char, int position) {
    ...
    if (char == MINUS) {
      ...
      position++;
      if (position == length) {
        return beginChunkNumber(NUM_SIGN, start);
      }
      char = getChar(position);
    }
    int digit = char ^ CHAR_0;
    ...
    if (digit == 0) {
      position++;
      if (position == length) {
        return beginChunkNumber(NUM_ZERO, start);
      }
      char = getChar(position);
      digit = char ^ CHAR_0;
      ...
      do {
        if (digitCount >= 18) {
          ...
          int maxInt = BigInt.parse("-922337203685477580").toInt();
        ...
        position++;
        if (position == length) {
          return beginChunkNumber(NUM_DIGIT, start);
        }
        char = getChar(position);
        digit = char ^ CHAR_0;
      } while (digit <= 9);
    ...
    if (char == DECIMALPOINT) {
      ...
      position++;
      if (position == length) {
        return beginChunkNumber(NUM_DOT, start);
      }
      char = getChar(position);
      digit = char ^ CHAR_0;
      if (digit > 9) {
      ...
      do {
        ...
        position++;
        if (position == length) {
          return beginChunkNumber(NUM_DOT_DIGIT, start);
        }
        char = getChar(position);
        digit = char ^ CHAR_0;
      } while (digit <= 9);
    ...
    if ((char | 0x20) == CHAR_e) {
      ...
      position++;
      if (position == length) {
        return beginChunkNumber(NUM_E, start);
      }
      char = getChar(position);
      int expSign = 1;
      int exponent = 0;
      if (((char + 1) | 2) == 0x2e /*+ or -*/) {
        ...
        position++;
        if (position == length) {
          return beginChunkNumber(NUM_E_SIGN, start);
        }
        char = getChar(position);
      }
      digit = char ^ CHAR_0;
      ...
      do {
        ...
        position++;
        if (position == length) {
          return beginChunkNumber(NUM_E_DIGIT, start);
        }
        char = getChar(position);
        digit = char ^ CHAR_0;
      } while (digit <= 9);
    ...
    if (doubleValue < maxExactDouble) {
      ...
      if (exponent >= -22) {
        if (exponent < 0) {
          listener.handleNumber(signedMantissa / _POWERS_OF_TEN[-exponent]);
          return position;
        }
        if (exponent == 0) {
